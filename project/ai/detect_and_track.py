...
def line_enter_check_and_set(loop,track,tp1,tp2,line_start,line_end):
    if isIntersect(tp1,tp2,line_start,line_end):
        if loop["id"] not in track.aoi_entered :
            ...
            append_to_file(str(save_dir)+f"/{opt.filename}.txt",msg)
...
#    print(cnts)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            # save_path = str(opt.filepath / opt.filename)
            save_path = str(Path(save_dir) / f"{opt.filename}.mp4")
    ...
    res1='/'+str(save_dir).replace("\\",'/')+'/'+f"/{opt.filename}.txt"
    res2= '/'+str(save_path).replace("\\",'/')+'/'+opt.filename+f'/{opt.filename}.mp4'
    print(res1,res2)
    return res1,res2
...
def mymain(cmd = False, custom_arg=None):
    ...
    # parser.add_argument('--project', default='../static/result', help='save results to project/name')
